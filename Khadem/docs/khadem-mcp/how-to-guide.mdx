---
title: "How To Guide"
description: "Connect Khadem as an MCP server to Claude Code or Claude Desktop"
---

## How to use `Khadem-mcp` on Claude Code

1. Download binary from [Khadem.com](https://Khadem.com)
2. Open Khadem and from a new tab click the settings icon to open the settings page.
...
4. In your terminal, type the below command (Replace `<mcp_url>` with the MCP URL you copied above): 
   ```
   claude mcp add --transport http khadem <mcp_url>
   # example: claude mcp add --transport http khadem http://127.0.0.1:9226/mcp
   ``` 
5. Now start Claude Code: `claude --dangerously-skip-permissions` (so Claude doesn't ask for confirmation each time)
6. Now, in Claude Code, type `Open amazon.com on khadem` to open the tab in Khadem.
...
## How to add `Khadem-mcp` on Claude Desktop

1. Download binary from [Khadem.com](https://Khadem.com)
2. Open Khadem and from a new tab click the settings icon to open the settings page. From the settings page, navigate to **MCP** in the sidebar and note the port number (usually `9225`).
3. Open your Claude Desktop config file: `/Users/<username>/Library/Application Support/Claude/claude_desktop_config.json`
4. Add Khadem to your config (replace the port with the value shown in MCP settings page):
```json
{
  "mcpServers": {
    "browserOS": {
...
```
5. Restart Claude Desktop. You should see the MCP connection indicator showing Khadem is connected
...
### claude-code + `Khadem-mcp` == agentic browser!
