---
title: "Setting up LM Studio"
description: "Configure LM Studio to run local LLMs with Khadem"
icon: "desktop"
---

## Setup Guide

<Steps>
  <Step title="Open LM Studio">
    Open LM Studio, navigate to the **Developer** page and select the desired model to load.

    <Frame>
      ![Open LM Studio](/images/setting-up-lm-studio/lmstudio-step1.png)
    </Frame>
  </Step>
  <Step title="Load the Model">
    In this example, we're loading the `openai/gpt-oss-20b` model.

    <Frame>
      ![Load the model](/images/setting-up-lm-studio/lmstudio-step2.png)
    </Frame>
  </Step>
  <Step title="Configure in Khadem">
    - Navigate to `chrome://settings/khadem` or go to **Settings → Khadem AI**
    - Click **Add Provider**
    - Select **OpenAI Compatible** in the Provider Type dropdown

    <Frame>
      ![Configure in Khadem](/images/setting-up-lm-studio/lmstudio-step3.png)
    </Frame>
  </Step>
  <Step title="Configure the Model">
    - Replace the Base URL with LM Studio URL (default: `http://localhost:1234/v1/`)
    - Set the Model ID to the one you loaded in LM Studio
    - **Important:** Set the **Context Window Size** to match your LM Studio configuration

    <Frame>
      ![Configure Base URL](/images/setting-up-lm-studio/lmstudio-step4-1.png)
    </Frame>
    <Frame>
      ![Configure Context Window](/images/setting-up-lm-studio/lmstudio-step4-2.png)
    </Frame>
  </Step>
  <Step title="Use the Model">
    Select the model in Khadem agent and start using it! 🎉

    <Frame>
      ![Use the model in Khadem](/images/setting-up-lm-studio/lmstudio-step5.png)
    </Frame>
  </Step>
</Steps>

<Warning>
  Ensure the Context Window Size in Khadem matches what you configured in LM Studio to avoid issues with long conversations.
</Warning>

## Configuration Details

<Accordion title="Default LM Studio Settings">
  - **Base URL**: `http://localhost:1234/v1/`
  - **Provider Type**: OpenAI Compatible
  - **Context Window**: Depends on your model and hardware (typically 2048-32768)
</Accordion>

<Accordion title="Supported Models">
  LM Studio supports various models including:

  - OpenAI GPT-OSS series
  - LLaMA and derivatives
  - Mistral models
  - And many more from Hugging Face
</Accordion>