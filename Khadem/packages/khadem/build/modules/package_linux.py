#!/usr/bin/env python3
"""
Linux packaging module for Khadem (AppImage and .deb)
"""
...
def copy_browser_files(
    ...
    dirs_to_copy = ["locales", "MEIPreload", "KhademServer"]
    ...
    khadem_path = Path(join_paths(target_dir, ctx.NXTSCAPE_APP_NAME))
    if khadem_path.exists():
        khadem_path.chmod(0o755)
...
"""
    ...
    desktop_file = Path(join_paths(apps_dir, "khadem.desktop"))
...
def copy_icon(ctx: BuildContext, icons_dir: Path) -> bool:
    ...
    icon_dest = Path(join_paths(icons_dir, "256x256", "apps", "khadem.png"))
...
def prepare_appdir(ctx: BuildContext, appdir: Path) -> bool:
    ...
    app_root = join_paths(appdir, "opt", "khadem")
    ...
    desktop_file = create_desktop_file(
        apps_dir, f"/opt/khadem/{ctx.NXTSCAPE_APP_NAME}"
    )
...
THIS="$(readlink -f "${{0}}")"
HERE="$(dirname "${{THIS}}")"
export LD_LIBRARY_PATH="${{HERE}}"/opt/khadem:$LD_LIBRARY_PATH
export CHROME_WRAPPER="${{THIS}}"
"${{HERE}}"/opt/khadem/{ctx.NXTSCAPE_APP_NAME} "$@"
"""
...
def create_launcher_script(ctx: BuildContext, bin_dir: Path) -> None:
    """Create launcher script in /usr/bin/khadem."""
    bin_dir.mkdir(parents=True, exist_ok=True)

    launcher_content = f"""#!/bin/sh
# Khadem launcher script
export LD_LIBRARY_PATH=/usr/lib/khadem:$LD_LIBRARY_PATH
exec /usr/lib/khadem/{ctx.NXTSCAPE_APP_NAME} "$@"
"""

    launcher_path = Path(join_paths(bin_dir, "khadem"))
...
Maintainer: Khadem Team <support@khadem.com>
Homepage: https://www.khadem.com/
Description: Khadem - The open source agentic browser
 Khadem is a privacy-focused web browser built on Chromium,
...
# Post-installation script for Khadem
set -e

# Set SUID bit on chrome_sandbox for sandboxing support
if [ -f /usr/lib/khadem/chrome_sandbox ]; then
    chmod 4755 /usr/lib/khadem/chrome_sandbox
fi
...
def prepare_debdir(ctx: BuildContext, debdir: Path) -> bool:
    ...
    │   │   └── [all browser files]
    │   └── share/
    │       ├── applications/khadem.desktop
    │       └── icons/hicolor/256x256/apps/khadem.png
    """
    log_info("📁 Preparing .deb directory structure...")

    lib_dir = join_paths(debdir, "usr", "lib", "khadem")
    ...
    # Create desktop file
    create_desktop_file(apps_dir, "/usr/bin/khadem")
...
def package_appimage(ctx: BuildContext, package_dir: Path) -> Optional[Path]:
    ...
    version = ctx.get_khadem_chromium_version().replace(" ", "_")
...
def package_deb(ctx: BuildContext, package_dir: Path) -> Optional[Path]:
    ...
    version = (
        ctx.get_khadem_chromium_version()
        .lstrip("v")
        .replace(" ", "")
        .replace("_", ".")
    )
    arch_suffix = "amd64" if ctx.architecture == "x64" else "arm64"
    filename = f"khadem_{version}_{arch_suffix}.deb"
    output_path = Path(join_paths(package_dir, filename))
...
def package(ctx: BuildContext) -> bool:
    """Package Khadem for Linux as both AppImage and .deb"""
    log_info(
        f"📦 Packaging {ctx.NXTSCAPE_APP_BASE_NAME} {ctx.get_khadem_chromium_version()} for Linux ({ctx.architecture})"
    )
