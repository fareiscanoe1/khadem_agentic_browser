#!/usr/bin/env python3
"""
Application signing and notarization module for Khadem
...
# Central list of Khadem Server binaries we need to sign explicitly.
...
KHADEM_SERVER_BINARIES: Dict[str, Dict[str, str]] = {
    "khadem_server": {
        "identifier_suffix": "khadem_server",
        "options": "runtime",
        "entitlements": "khadem-executable-entitlements.plist",
    },
    "codex": {
...
def get_khadem_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known Khadem Server binaries, if applicable."""
    name = component_path.stem.lower()
    return KHADEM_SERVER_BINARIES.get(name)
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for Khadem Framework
    # Handle both release and debug framework names
    framework_names = [
        "Khadem Framework.framework",
        "Khadem Dev Framework.framework",
    ]
    khadem_framework_paths = []

    for fw_name in framework_names:
        fw_path = join_paths(framework_path, fw_name)
        if fw_path.exists():
            khadem_framework_paths.append(fw_path)

            # Add versioned path if context is available
            if ctx and ctx.khadem_chromium_version:
                versioned_path = join_paths(
                    fw_path, "Versions", ctx.khadem_chromium_version
                )
                if versioned_path.exists():
                    khadem_framework_paths.insert(
                        0, versioned_path
                    )  # Prioritize versioned path

    # Find all helper apps
    for khadem_fw_path in khadem_framework_paths:
        helpers_dir = join_paths(khadem_fw_path, "Helpers")
    ...
    # Find all dylibs (check versioned path for Khadem Framework libraries)
    for khadem_fw_path in khadem_framework_paths:
        libraries_dir = join_paths(khadem_fw_path, "Libraries")
    ...
    # Find Khadem Server binaries
    khadem_server_dir = join_paths(app_path, "Contents", "Resources", "KhademServer")
    if khadem_server_dir.exists():
        for item in khadem_server_dir.rglob("*"):
            if item.is_file() and not item.suffix and os.access(item, os.X_OK):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.khadem"
) -> str:
    ...
    # Khadem Server binaries share the same entitlements/options but need unique identifiers.
    khadem_server_info = get_khadem_server_binary_info(component_path)
    if khadem_server_info:
        suffix = khadem_server_info.get("identifier_suffix", component_path.stem)
    ...
    if component_path.suffix == ".framework":
        if name == "Khadem Framework" or name == "Khadem Dev Framework":
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known Khadem Server binaries share the same relaxed options.
    khadem_server_info = get_khadem_server_binary_info(component_path)
    if khadem_server_info:
        return khadem_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            khadem_server_info = get_khadem_server_binary_info(exe)
            if khadem_server_info:
                entitlements_name = khadem_server_info.get("entitlements")
    ...
    # 6. Sign frameworks (except the main Khadem Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before Khadem Framework.framework
    ...
    main_exe_names = ["Khadem", "Khadem Dev"]
    main_exe = None
    ...
    if not sign_component(main_exe, certificate_name, "com.khadem.Khadem"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.khadem.Khadem" and '
...
def sign_app(ctx: BuildContext, create_dmg: bool = True) -> bool:
    ...
    log_info("=" * 70)
    log_info("🚀 Starting signing process for Khadem...")
    log_info("=" * 70)
