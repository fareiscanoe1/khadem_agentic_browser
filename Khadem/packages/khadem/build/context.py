...
class BuildContext:
    ...
    khadem_version: str = ""
    khadem_chromium_version: str = ""
    start_time: float = 0.0

    # App names - will be set based on platform
    CHROMIUM_APP_NAME: str = ""
    NXTSCAPE_APP_NAME: str = ""
    NXTSCAPE_APP_BASE_NAME: str = "Khadem"  # Base name without extension
    ...
    def __post_init__(self):
        ...
        if not self.khadem_version:
            # Read from NXTSCAPE_VERSION file
            version_file = join_paths(
                self.root_dir, "build", "config", "NXTSCAPE_VERSION"
            )
            if version_file.exists():
                self.khadem_version = version_file.read_text().strip()

        # Set khadem_chromium_version as chromium version with BUILD + khadem_version
        if self.chromium_version and self.khadem_version and version_dict:
            # Calculate new BUILD number by adding khadem_version to original BUILD
            new_build = int(version_dict["BUILD"]) + int(self.khadem_version)
            self.khadem_chromium_version = f"{version_dict['MAJOR']}.{version_dict['MINOR']}.{new_build}.{version_dict['PATCH']}"
    ...
    def get_dmg_name(self, signed=False) -> str:
        ...
        if self.architecture == "universal":
            if signed:
                return f"{self.NXTSCAPE_APP_BASE_NAME}_{self.khadem_chromium_version}_universal_signed.dmg"
            return f"{self.NXTSCAPE_APP_BASE_NAME}_{self.khadem_chromium_version}_universal.dmg"
        else:
            if signed:
                return f"{self.NXTSCAPE_APP_BASE_NAME}_{self.khadem_chromium_version}_{self.architecture}_signed.dmg"
            return f"{self.NXTSCAPE_APP_BASE_NAME}_{self.khadem_chromium_version}_{self.architecture}.dmg"

    def get_khadem_chromium_version(self) -> str:
        """Get Khadem version string"""
        return self.khadem_chromium_version

    def get_khadem_version(self) -> str:
        """Get Khadem version string"""
        return self.khadem_version
    ...
    def get_dist_dir(self) -> Path:
        ...
        return join_paths(self.root_dir, "dist", self.khadem_version)
