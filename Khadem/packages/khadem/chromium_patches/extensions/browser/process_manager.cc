...
+  // Khadem: Add permanent keepalive for Khadem extensions to prevent
+  // their service workers from being terminated due to inactivity.
+  if (khadem::IsKhademExtension(worker_id.extension_id)) {
+    base::Uuid keepalive_uuid = IncrementServiceWorkerKeepaliveCount(
+        worker_id,
+        content::ServiceWorkerExternalRequestTimeoutType::kDoesNotTimeout,
+        Activity::PROCESS_MANAGER,
+        "khadem_permanent_keepalive");
+    khadem_permanent_keepalives_[worker_id] = keepalive_uuid;
+    VLOG(1) << "khadem: Added permanent keepalive for extension "
...
+  // Khadem: Clean up permanent keepalive for Khadem extensions.
+  auto keepalive_iter = khadem_permanent_keepalives_.find(worker_id);
+  if (keepalive_iter != khadem_permanent_keepalives_.end()) {
+    DecrementServiceWorkerKeepaliveCount(
+        worker_id, keepalive_iter->second, Activity::PROCESS_MANAGER,
+        "khadem_permanent_keepalive");
+    khadem_permanent_keepalives_.erase(keepalive_iter);
+    VLOG(1) << "khadem: Removed permanent keepalive for extension "
+            << worker_id.extension_id;
+  }
+
