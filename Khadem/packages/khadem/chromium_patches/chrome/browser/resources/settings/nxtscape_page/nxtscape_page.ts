...
+    if (this.prefs && this.prefs.khadem) {
+      this.loadProviders_();
+    }
+  }
+
+  private loadProviders_() {
+    // Load from preferences or initialize with Khadem
+    if (!this.prefs || !this.prefs.khadem || !this.prefs.khadem.providers) {
+      // Prefs not ready yet, initialize defaults locally only
+      this.initializeDefaultProvidersLocally_();
+      return;
+    }
+    
+    const stored = this.getPref('khadem.providers');
...
+        
+        // Ensure Khadem is always present
+        if (!this.providers_.some(p => p.id === 'khadem')) {
...
+    
+    const khadem: ProviderConfig = {
+      id: 'khadem',
+      name: 'Khadem',
+      type: ProviderType.KHADEM,
+      isDefault: true,
+      isBuiltIn: true,
+      createdAt: now,
+      updatedAt: now,
+    };
+    
+    this.providers_ = [khadem];
+    this.defaultProviderId_ = 'khadem';
...
+    if (this.prefs && this.prefs.khadem && this.prefs.khadem.providers !== undefined) {
+      this.saveProviders_();
+    }
+  }
+
...
+    if (!this.prefs || !this.prefs.khadem || this.prefs.khadem.providers === undefined) {
+      console.warn('khadem: Cannot save providers - prefs not ready');
+      return;
+    }
+    
...
+    this.setPrefValue('khadem.providers', JSON.stringify(data));
+    console.log('khadem: Saving providers:', data);
+  }
+
...
+    console.log('khadem: Add Provider clicked');
+    
...
+    }
+    
+    console.log('khadem: Form visibility:', this.showProviderForm_);
+  }
+
...
+    const provider = this.providers_.find(p => p.id === providerId);
+    
+    console.log('khadem: Delete provider clicked:', providerId, provider);
+    
...
+      
+      // If deleted provider was default, set Khadem as default
+      if (provider.id === this.defaultProviderId_) {
+        this.set('defaultProviderId_', 'khadem');
...
+    }
+    
+    console.log('khadem: Provider type changed to:', providerType);
+    
+    const defaults = PROVIDER_DEFAULTS[providerType];
+    if (defaults) {
+      console.log('khadem: Applying defaults:', defaults);
...
+            console.log('khadem: Auto-set context length to:', contextLength);
+          }
+          
+          this.set('showModelDropdown_', false);
+          this.set('selectedSuggestionIndex_', -1);
...
+    console.log('khadem: Model suggestion clicked:', model);
+    
+    if (model) {
+      console.log('khadem: Setting dialogModelId_ to:', model);
...
+      if (input) {
+        input.value = model;
+        console.log('khadem: Input value set to:', input.value);
+      }
+    }
+  }
+  
...
+    console.log('khadem: Custom model selected:', this.dialogModelId_);
+    
...
+      
+      chrome.send('logKhademMetric', ['settings.provider.added', {
...
+    console.log('khadem: Validating form:', {
+      name: this.dialogProviderName_,
+      type: this.dialogProviderType_,
+      apiKey: this.dialogApiKey_,
+      modelId: this.dialogModelId_
...
+
+    chrome.send('logKhademMetric', ['settings.default_provider.changed', {
...
+      classes += ' selected';
+    }
+    if (provider.type === ProviderType.KHADEM) {
+      classes += ' khadem';
+    }
+    return classes;
+  }
...
+    }, 600);
+    
+    console.log('khadem: Using template:', template.name);
+  }
+
