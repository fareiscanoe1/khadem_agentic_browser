diff --git a/chrome/browser/resources/settings/khadem_prefs_page/khadem_prefs_page.ts b/chrome/browser/resources/settings/khadem_prefs_page/khadem_prefs_page.ts
new file mode 100644
index 0000000000000..2616c77cc001b
--- /dev/null
+++ b/chrome/browser/resources/settings/khadem_prefs_page/khadem_prefs_page.ts
@@ -0,0 +1,250 @@
...
+
+/**
+ * @fileoverview 'settings-khadem-prefs-page' contains Khadem-specific settings.
+ */
+
...
+
+import {getTemplate} from './khadem_prefs_page.html.js';
+
...
+}
+
+export interface SettingsKhademPrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const SettingsKhademPrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class SettingsKhademPrefsPageElement extends SettingsKhademPrefsPageElementBase {
+  static get is() {
+    return 'settings-khadem-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('khadem.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('khadem.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+    'settings-khadem-prefs-page': SettingsKhademPrefsPageElement;
+  }
+}
+
+customElements.define(
+    SettingsKhademPrefsPageElement.is, SettingsKhademPrefsPageElement);
