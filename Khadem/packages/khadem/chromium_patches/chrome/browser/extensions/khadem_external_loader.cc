diff --git a/chrome/browser/extensions/khadem_external_loader.cc b/chrome/browser/extensions/khadem_external_loader.cc
new file mode 100644
index 0000000000000..53eeebf1be954
--- /dev/null
+++ b/chrome/browser/extensions/khadem_external_loader.cc
@@ -0,0 +1,661 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#include "chrome/browser/extensions/khadem_external_loader.h"
+
+#include <memory>
+#include <utility>
+
...
+namespace {
+
+// Default config URL - should be updated to actual Khadem server
+// Can be overridden via --khadem-extensions-url command line flag
+constexpr char kKhademConfigUrl[] = "https://cdn.khadem.com/extensions/extensions.json";
+
...
+constexpr net::NetworkTrafficAnnotationTag kKhademExtensionsFetchTrafficAnnotation =
+    net::DefineNetworkTrafficAnnotation("khadem_extensions_fetch", R"(
+        semantics {
+          sender: "Khadem External Extension Loader"
+          description:
+            "Fetches a JSON configuration file that specifies which extensions "
+            "should be installed for Khadem users at startup."
+          trigger:
+            "Triggered during browser startup when Khadem mode is enabled."
+          data:
+            "No user data is sent. Only a GET request to fetch the configuration."
+          destination: OTHER
+          destination_other:
+            "The Khadem configuration server specified by the config URL."
+        }
+        policy {
+          cookies_allowed: NO
+          setting:
...
+            "Not implemented yet. This is a new feature for Khadem."
+        })");
+
+// Example JSON format:
+// {
...
+}  // namespace
+
+KhademExternalLoader::KhademExternalLoader(Profile* profile)
+    : profile_(profile) {
+  // Default config URL - can be overridden via SetConfigUrl
+  config_url_ = GURL(kKhademConfigUrl);
+  
+  // Add known Khadem extension IDs
+  for (const char* extension_id : khadem::kAllowedExtensions) {
+    khadem_extension_ids_.insert(extension_id);
+  }
+}
+
+KhademExternalLoader::~KhademExternalLoader() = default;
+
+void KhademExternalLoader::StartLoading() {
+  LOG(INFO) << "Khadem external extension loader starting...";
+  
...
+    LOG(ERROR) << "Invalid Khadem extensions config URL";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+  
+  LOG(INFO) << "Fetching Khadem extensions from: " << config_url_.spec();
+
...
+      std::move(resource_request), kKhademExtensionsFetchTrafficAnnotation);
+
+  // Start the download
+  url_loader_->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
+      url_loader_factory_.get(),
+      base::BindOnce(&KhademExternalLoader::OnURLFetchComplete,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void KhademExternalLoader::OnURLFetchComplete(
+    std::unique_ptr<std::string> response_body) {
+  if (!response_body) {
+    LOG(ERROR) << "Failed to fetch Khadem extensions config from " 
+               << config_url_.spec();
+    LoadFinished(base::Value::Dict());
+    return;
+  }
...
+}
+
+void KhademExternalLoader::ParseConfiguration(
+    const std::string& json_content) {
+  std::optional<base::Value> parsed_json = base::JSONReader::Read(json_content);
+  
+  if (!parsed_json || !parsed_json->is_dict()) {
+    LOG(ERROR) << "Failed to parse Khadem extensions config JSON";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+
...
+  
+  if (!extensions_dict) {
+    LOG(ERROR) << "No 'extensions' key found in Khadem config";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+
...
+  }
+  
+  LOG(INFO) << "Loaded " << prefs.size() << " extensions from Khadem config";
+  
...
+  
+  // Immediately trigger high-priority installation of all Khadem extensions
...
+  if (!khadem_extension_ids_.empty()) {
+    LOG(INFO) << "khadem: Triggering immediate high-priority installation for " 
+              << khadem_extension_ids_.size() << " Khadem extensions";
+    
+    // Use a delayed task to ensure the extension system is fully initialized
+    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+        FROM_HERE,
+        base::BindOnce(&KhademExternalLoader::TriggerImmediateInstallation,
+                       weak_ptr_factory_.GetWeakPtr()),
...
+}
+
+void KhademExternalLoader::StartPeriodicCheck() {
+  LOG(INFO) << "khadem: Starting periodic maintenance (every " 
+            << kPeriodicMaintenanceInterval.InMinutes() << " minutes)";
+  
+  // Schedule the periodic maintenance
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&KhademExternalLoader::PeriodicMaintenance,
+                     weak_ptr_factory_.GetWeakPtr()),
+      kPeriodicMaintenanceInterval);
+}
+
+void KhademExternalLoader::PeriodicMaintenance() {
+  LOG(INFO) << "khadem: Running periodic maintenance";
+  
+  if (!profile_) {
...
+  }
+  
+  // 1. Check for and reinstall any uninstalled Khadem extensions
+  ReinstallUninstalledExtensions();
+  
+  // 2. Re-enable any disabled Khadem extensions
+  ReenableDisabledExtensions();
+  
+  // 3. Fetch latest config and check for changes
+  FetchAndCheckConfig();
+  
+  // 4. Force immediate update check for all Khadem extensions
+  ForceUpdateCheck();
+
...
+}
+
+void KhademExternalLoader::ReinstallUninstalledExtensions() {
...
+  }
+  
+  for (const std::string& extension_id : khadem_extension_ids_) {
...
+    }
+    
+    LOG(INFO) << "khadem: Extension " << extension_id 
...
+    if (!extension_config) {
+      LOG(WARNING) << "khadem: No config found for " << extension_id;
+      continue;
+    }
+    
...
+    if (!update_url) {
+      LOG(WARNING) << "khadem: No update URL found for " << extension_id;
+      continue;
+    }
+    
...
+      LOG(WARNING) << "khadem: Invalid update URL for " << extension_id;
+      continue;
+    }
+    
...
+        false);
+    
+    LOG(INFO) << "khadem: Added " << extension_id 
...
+      LOG(INFO) << "khadem: Triggered immediate install for " << extension_id;
+    }
+  }
+}
+
+void KhademExternalLoader::ReenableDisabledExtensions() {
+  ExtensionService* service = ExtensionSystem::Get(profile_)->extension_service();
+  if (!service) {
+    return;
+  }
...
+    }
+
+    // Re-enable Khadem extensions regardless of disable reason
+    LOG(INFO) << "khadem: Re-enabling extension " << extension_id;
+    service->EnableExtension(extension_id);
+  }
+}
+
+void KhademExternalLoader::FetchAndCheckConfig() {
+  LOG(INFO) << "khadem: Fetching latest config to check for changes";
+  
...
+        url_loader_factory_.get(),
+        base::BindOnce(&KhademExternalLoader::OnConfigCheckComplete,
+                       weak_ptr_factory_.GetWeakPtr(),
+                       std::move(config_check_loader)));
+  }
+}
+
+void KhademExternalLoader::OnConfigCheckComplete(
+    std::unique_ptr<network::SimpleURLLoader> loader,
+    std::unique_ptr<std::string> response_body) {
+  if (!response_body) {
+    LOG(WARNING) << "khadem: Failed to fetch config for update check";
+    return;
+  }
+  
+  std::optional<base::Value> parsed_json = base::JSONReader::Read(*response_body);
+  if (!parsed_json || !parsed_json->is_dict()) {
+    LOG(WARNING) << "khadem: Invalid config JSON during update check";
+    return;
+  }
+  
+  const base::Value::Dict* extensions_dict = 
...
+        config_changed = true;
+        LOG(INFO) << "khadem: Config changed for extension " << extension_id;
+        break;
+      }
+    }
+    
...
+        config_changed = true;
+        LOG(INFO) << "khadem: Extension " << extension_id << " removed from config";
+        break;
+      }
+    }
+  }
+  
+  if (config_changed) {
+    LOG(INFO) << "khadem: Config has changed, reloading extensions";
+    
...
+    ParseConfiguration(*response_body);
+  } else {
+    LOG(INFO) << "khadem: Config unchanged";
+  }
+}
+
+void KhademExternalLoader::TriggerImmediateInstallation() {
+  if (!profile_ || khadem_extension_ids_.empty()) {
+    return;
+  }
+  
+  LOG(INFO) << "khadem: Triggering immediate installation on first start";
+  
...
+      if (registry->GetInstalledExtension(extension_id)) {
+        LOG(INFO) << "khadem: Extension " << extension_id << " already installed";
+        continue;
+      }
+      
...
+  ExtensionUpdater* updater = ExtensionUpdater::Get(profile_);
+  if (!updater) {
+    LOG(WARNING) << "khadem: No extension updater available for immediate installation";
+    return;
+  }
+  
+  LOG(INFO) << "khadem: Executing CheckNow with immediate install for " 
+            << khadem_extension_ids_.size() << " Khadem extensions";
+  
+  // Create CheckParams for immediate foreground installation
+  ExtensionUpdater::CheckParams params;
+  params.ids = std::list<ExtensionId>(khadem_extension_ids_.begin(), 
+                                       khadem_extension_ids_.end());
+  params.install_immediately = true;
+  params.fetch_priority = DownloadFetchPriority::kForeground;
+  
...
+}
+
+void KhademExternalLoader::ForceUpdateCheck() {
+  if (!profile_ || khadem_extension_ids_.empty()) {
+    return;
+  }
+  
+  ExtensionUpdater* updater = ExtensionUpdater::Get(profile_);
+  if (!updater) {
+    LOG(WARNING) << "khadem: No extension updater available";
+    return;
+  }
+  
+  LOG(INFO) << "khadem: Forcing immediate update check for " 
+            << khadem_extension_ids_.size() << " Khadem extensions";
+  
...
+}
+
+void KhademExternalLoader::LoadFromFile() {
...
+          LOG(ERROR) << "Failed to read Khadem config file: " << path;
+          return std::string();
+        }
+        return contents;
+      }, config_file_for_testing_),
+      base::BindOnce(&KhademExternalLoader::ParseConfiguration,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void KhademExternalLoader::CheckAndLogExtensionState(
...
+
+    // Log to metrics
+    khadem_metrics::KhademMetrics::Log("ota.extension.unexpected_state",
+                                              std::move(properties));
+
+    // Also log to Chrome logs for local debugging
+    LOG(WARNING) << "khadem: Extension " << extension_id
