diff --git a/chrome/browser/extensions/khadem_external_loader.h b/chrome/browser/extensions/khadem_external_loader.h
new file mode 100644
index 0000000000000..3d0ba110fe6e1
--- /dev/null
+++ b/chrome/browser/extensions/khadem_external_loader.h
@@ -0,0 +1,116 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_EXTENSIONS_KHADEM_EXTERNAL_LOADER_H_
+#define CHROME_BROWSER_EXTENSIONS_KHADEM_EXTERNAL_LOADER_H_
+
+#include <memory>
+#include <string>
+
...
+// from a remote URL. This is designed for Khadem to specify a set of
+// extensions that should be installed at startup.
+class KhademExternalLoader : public ExternalLoader {
+ public:
+  explicit KhademExternalLoader(Profile* profile);
+
+  KhademExternalLoader(const KhademExternalLoader&) = delete;
+  KhademExternalLoader& operator=(const KhademExternalLoader&) = delete;
+
...
+  }
+
+  // Starts periodic check to re-enable disabled Khadem extensions and check for updates
...
+  
+  // Forces immediate update check for Khadem extensions
+  void ForceUpdateCheck();
+
+ protected:
+  ~KhademExternalLoader() override;
+
+  // ExternalLoader:
...
+  void LoadFromFile();
+  
+  // Checks for uninstalled Khadem extensions and reinstalls them
+  void ReinstallUninstalledExtensions();
+  
+  // Re-enables Khadem extensions that were disabled by user action
+  void ReenableDisabledExtensions();
+  
+  // Triggers immediate installation of all Khadem extensions on first start
...
+
+  // List of Khadem extension IDs to monitor
+  std::set<std::string> khadem_extension_ids_;
+  
+  // Last fetched config for comparison
+  base::Value::Dict last_config_;
+
+  base::WeakPtrFactory<KhademExternalLoader> weak_ptr_factory_{this};
+};
+
+}  // namespace extensions
+
+#endif  // CHROME_BROWSER_EXTENSIONS_KHADEM_EXTERNAL_LOADER_H_
