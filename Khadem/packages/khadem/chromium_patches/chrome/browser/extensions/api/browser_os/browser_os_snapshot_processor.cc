...
+            &is_offscreen);
+        
+        VLOG(3) << "[khadem] Node " << node_data.id 
...
+    } else {
+      // No AXTree available
+      LOG(WARNING) << "[khadem] No AXTree available for bounds computation";
+    }
+    
...
+  }
+  
+  LOG(INFO) << "[khadem] Viewport: " << viewport_size.ToString() 
...
+  
+  if (!ax_tree) {
+    LOG(ERROR) << "[khadem] Failed to create AXTree from update";
+    SnapshotProcessingResult result;
...
+  }
+  
+  LOG(INFO) << "[khadem] Created AXTree with " << tree_update.nodes.size() 
