...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[khadem] No RenderFrameHost for AccessibilityDoDefault";
+    return false;
+  }
+  
...
+  
+  LOG(INFO) << "[khadem] Performing AccessibilityDoDefault on node " 
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[khadem] No RenderFrameHost for AccessibilityFocus";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  action_data.target_tree_id = node_info.ax_tree_id;
+  
+  LOG(INFO) << "[khadem] Performing AccessibilityFocus on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[khadem] No RenderFrameHost for AccessibilityScrollToMakeVisible";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[khadem] Performing AccessibilityScrollToMakeVisible on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+    LOG(INFO) << "[khadem] Node is out of viewport, scrolling to make visible";
...
+    // LOG(INFO) << "[khadem] Node was out of viewport, trying AccessibilityDoDefault click first";
+    // bool changed = KhademChangeDetector::ExecuteWithDetection(
+    //     web_contents,
...
+      LOG(INFO) << "[khadem] No change from accessibility click, trying HTML click";
+      changed = KhademChangeDetector::ExecuteWithDetection(
+          web_contents,
+          [&]() { HtmlClick(web_contents, node_info); },
+          base::Milliseconds(200));
+    }
+    
+    LOG(INFO) << "[khadem] Click result: " << (changed ? "changed" : "no change");
+    return changed;
+  }
+  
+  // For in-viewport nodes, try coordinate click first (most natural)
+  LOG(INFO) << "[khadem] Node is in viewport, trying coordinate click first";
+  gfx::PointF click_point = GetNodeCenterPoint(web_contents, node_info);
+  
+  bool changed = KhademChangeDetector::ExecuteWithDetection(
+      web_contents,
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[khadem] No RenderFrameHost for AccessibilitySetValue";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[khadem] Performing AccessibilitySetValue on node " 
...
+  
+  if (is_out_of_viewport) {
+    LOG(INFO) << "[khadem] Node is out of viewport for typing, scrolling to make visible";
+    AccessibilityScrollToMakeVisible(web_contents, node_info, true /* center */);
+    // Wait for scroll to complete
+    base::PlatformThread::Sleep(base::Milliseconds(300));
+  }
...
+  if (!changed) {
+    LOG(INFO) << "[khadem] No change from native typing, trying JavaScript";
+    changed = KhademChangeDetector::ExecuteWithDetection(
+        web_contents,
...
+  // if (!changed) {
+  //   LOG(INFO) << "[khadem] No change from JavaScript, trying accessibility SetValue";
+  //   changed = KhademChangeDetector::ExecuteWithDetection(
+  //       web_contents,
+  //       [&]() {
...
+  // }
+  
+  LOG(INFO) << "[khadem] Type result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[khadem] Clear result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[khadem] KeyPress result for '" << key << "': " 
...
+    LOG(INFO) << "[khadem] No interactive elements in viewport to highlight";
+    return;
+  }
+  
+  LOG(INFO) << "[khadem] Highlighting " << filtered_nodes.size() 
...
+      document.querySelectorAll('.khadem-bbox').forEach(e => e.remove());
+      
+      // Create container for all bounding boxes
+      const container = document.createElement('div');
+      container.className = 'khadem-bbox-container';
...
+      document.querySelectorAll('.khadem-bbox-container').forEach(e => e.remove());
+      document.querySelectorAll('.khadem-bbox').forEach(e => e.remove());
+      
+      // Remove all highlight containers  
+      document.querySelectorAll('.khadem-highlight-container').forEach(e => e.remove());
+      document.querySelectorAll('.khadem-highlight').forEach(e => e.remove());
+      
+      // Remove any style elements we added
+      document.querySelectorAll('#khadem-highlight-styles').forEach(e => e.remove());
+      
+      return true;
+    })();
+  )";
...
+                                   const gfx::PointF& point) {
+  LOG(INFO) << "[khadem] ClickCoordinatesWithDetection at (" 
+            << point.x() << ", " << point.y() << ")";
+  
...
+      base::Milliseconds(300));
+  
+  LOG(INFO) << "[khadem] Click coordinates result: " 
+            << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+                                    const gfx::PointF& point,
+                                    const std::string& text) {
+  LOG(INFO) << "[khadem] TypeAtCoordinatesWithDetection at (" 
...
+  if (!changed) {
+    LOG(INFO) << "[khadem] No change from native typing at coordinates, trying JS injection";
+    
...
+  }
+  
+  LOG(INFO) << "[khadem] Type at coordinates result: " 
