...
+uint32_t KhademGetInteractiveSnapshotFunction::next_snapshot_id_ = 1;
+
+// Constructor and destructor implementations
+KhademGetInteractiveSnapshotFunction::KhademGetInteractiveSnapshotFunction() = default;
+KhademGetInteractiveSnapshotFunction::~KhademGetInteractiveSnapshotFunction() = default;
+
+ExtensionFunction::ResponseAction KhademGetAccessibilityTreeFunction::Run() {
...
+      base::BindOnce(
+          &KhademGetAccessibilityTreeFunction::OnAccessibilityTreeReceived,
+          this),
...
+}
+
+void KhademGetAccessibilityTreeFunction::OnAccessibilityTreeReceived(
...
+}
+
+// Implementation of KhademGetInteractiveSnapshotFunction
+
+ExtensionFunction::ResponseAction KhademGetInteractiveSnapshotFunction::Run() {
...
+    LOG(WARNING) << "[khadem] Frame not stable for AX snapshot - skipping";
...
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(
+          &KhademGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived,
+          this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties |
+                 ui::AXMode::kInlineTextBoxes),
+      /* max_nodes= */ 0,  // No limit
...
+}
+
+void KhademGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  // Double-check frame is still valid before processing
+  if (!web_contents_) {
+    LOG(WARNING) << "[khadem] WebContents gone during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+    LOG(WARNING) << "[khadem] Frame became unstable during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+      web_contents_,
+      base::BindOnce(
+          &KhademGetInteractiveSnapshotFunction::OnSnapshotProcessed,
+          base::WrapRefCounted(this)));
+}
+
+void KhademGetInteractiveSnapshotFunction::OnSnapshotProcessed(
+    SnapshotProcessingResult result) {
+  Respond(ArgumentList(
+      browser_os::GetInteractiveSnapshot::Results::Create(result.snapshot)));
+}
+
+// Implementation of KhademClickFunction
+
+ExtensionFunction::ResponseAction KhademClickFunction::Run() {
...
+}
+
+// Implementation of KhademInputTextFunction
+
+ExtensionFunction::ResponseAction KhademInputTextFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[khadem] InputText: Starting input for nodeId: " << params->node_id;
+  
...
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[khadem] InputText: No change detected after typing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of KhademClearFunction
+
+ExtensionFunction::ResponseAction KhademClearFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[khadem] Clear: Clearing field for nodeId: " << params->node_id;
+  
+  // Use ClearWithDetection which handles focus and clearing
+  bool change_detected = ClearWithDetection(web_contents, node_info);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[khadem] Clear: No change detected after clearing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of KhademGetPageLoadStatusFunction
+
+ExtensionFunction::ResponseAction KhademGetPageLoadStatusFunction::Run() {
...
+}
+
+// Implementation of KhademScrollUpFunction
+
+ExtensionFunction::ResponseAction KhademScrollUpFunction::Run() {
...
+}
+
+// Implementation of KhademScrollDownFunction
+
+ExtensionFunction::ResponseAction KhademScrollDownFunction::Run() {
...
+}
+
+// Implementation of KhademScrollToNodeFunction
+
+ExtensionFunction::ResponseAction KhademScrollToNodeFunction::Run() {
...
+}
+
+// Implementation of KhademSendKeysFunction
+
+ExtensionFunction::ResponseAction KhademSendKeysFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[khadem] SendKeys: Sending key '" << params->key << "'";
+  
+  // Send the key with change detection
+  bool change_detected = KeyPressWithDetection(web_contents, params->key);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[khadem] SendKeys: No change detected after key press";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of KhademCaptureScreenshotFunction
+
+KhademCaptureScreenshotFunction::KhademCaptureScreenshotFunction() = default;
+KhademCaptureScreenshotFunction::~KhademCaptureScreenshotFunction() = default;
+
+ExtensionFunction::ResponseAction KhademCaptureScreenshotFunction::Run() {
...
+    LOG(INFO) << "[khadem] CaptureScreenshot: Using exact dimensions: "
...
+      LOG(INFO) << "[khadem] CaptureScreenshot: Using thumbnail size: " << max_dimension 
...
+      LOG(INFO) << "[khadem] CaptureScreenshot: Using viewport size: " << max_dimension;
+    }
+    
...
+}
+
+void KhademCaptureScreenshotFunction::DrawHighlightsAndCapture() {
...
+      LOG(INFO) << "[khadem] Drawing highlights for screenshot with " 
+                << tab_it->second.size() << " interactive elements";
+      ShowHighlights(web_contents_, tab_it->second, true /* show_labels */);
+    } else {
+      LOG(INFO) << "[khadem] No snapshot data available for highlighting";
+    }
+    
...
+        FROM_HERE,
+        base::BindOnce(&KhademCaptureScreenshotFunction::CaptureScreenshotNow,
+                       base::WrapRefCounted(this)),
...
+}
+
+void KhademCaptureScreenshotFunction::CaptureScreenshotNow() {
+  if (!web_contents_) {
...
+      target_size_,
+      base::BindOnce(&KhademCaptureScreenshotFunction::OnScreenshotCaptured,
+                     this));
+}
+
+void KhademCaptureScreenshotFunction::OnScreenshotCaptured(
...
+}
+
+// KhademGetSnapshotFunction implementation
+ExtensionFunction::ResponseAction KhademGetSnapshotFunction::Run() {
...
+  // Request accessibility tree snapshot
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(&KhademGetSnapshotFunction::OnAccessibilityTreeReceived,
+                     this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties),
+      /* max_nodes= */ 0,  // No limit
+      /* timeout= */ base::TimeDelta(),
...
+}
+
+void KhademGetSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  if (!has_callback()) {
+    return;
+  }
...
+}
+
+// KhademGetPrefFunction
+ExtensionFunction::ResponseAction KhademGetPrefFunction::Run() {
...
+}
+
+// KhademSetPrefFunction
+ExtensionFunction::ResponseAction KhademSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying khadem.* prefs
+  if (!params->name.starts_with("khadem.")) {
+    return RespondNow(Error("Only khadem.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+// KhademGetAllPrefsFunction
+ExtensionFunction::ResponseAction KhademGetAllPrefsFunction::Run() {
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* profile_prefs = profile->GetPrefs();
+  PrefService* local_state = g_browser_process->local_state();
+
+  // Build a combined khadem prefs dict from both sources
+  base::Value::Dict combined_khadem;
+
+  // Lambda to merge khadem prefs from a PrefService
...
+
+    // Look for "khadem" key in the top-level dict
+    const base::Value* khadem_value = pref_dict.Find("khadem");
+    if (khadem_value && khadem_value->is_dict()) {
+      // Merge this khadem dict into combined
+      combined_khadem.Merge(khadem_value->GetDict().Clone());
+      LOG(INFO) << "[khadem] GetAllPrefs: Found khadem.* prefs in " << source_name;
+    }
+  };
+
...
+
+  // Create single PrefObject with the entire khadem dict
+  std::vector<browser_os::PrefObject> pref_objects;
+  browser_os::PrefObject pref_obj;
+  pref_obj.key = "khadem";
+  pref_obj.type = "dictionary";
+  pref_obj.value = base::Value(std::move(combined_khadem));
...
+}
+
+// KhademLogMetricFunction
+ExtensionFunction::ResponseAction KhademLogMetricFunction::Run() {
...
+    
+    khadem_metrics::KhademMetrics::Log(prefixed_event, std::move(properties));
+  } else {
+    // No properties, just log with extension ID
+    khadem_metrics::KhademMetrics::Log(prefixed_event, {
...
+}
+
+// KhademGetVersionNumberFunction
+ExtensionFunction::ResponseAction KhademGetVersionNumberFunction::Run() {
...
+}
+
+// KhademExecuteJavaScriptFunction
+ExtensionFunction::ResponseAction KhademExecuteJavaScriptFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[khadem] ExecuteJavaScript: Executing code in tab " << tab_info->tab_id;
+  
...
+      js_code,
+      base::BindOnce(&KhademExecuteJavaScriptFunction::OnJavaScriptExecuted,
+                     this),
...
+}
+
+void KhademExecuteJavaScriptFunction::OnJavaScriptExecuted(base::Value result) {
+  LOG(INFO) << "[khadem] ExecuteJavaScript: Execution completed";
+
+  if (result.is_none()) {
...
+}
+
+// Implementation of KhademClickCoordinatesFunction
+ExtensionFunction::ResponseAction KhademClickCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[khadem] ClickCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
...
+  
+  LOG(INFO) << "[khadem] ClickCoordinates: Clicking at (" 
+            << params->x << ", " << params->y << ")";
+  
...
+  
+  LOG(INFO) << "[khadem] ClickCoordinates: Result = " 
...
+}
+
+// Implementation of KhademTypeAtCoordinatesFunction  
+ExtensionFunction::ResponseAction KhademTypeAtCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[khadem] TypeAtCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
+    response.success = false;
+    return RespondNow(ArgumentList(
...
+  gfx::PointF click_point(params->x, params->y);
+  
+  LOG(INFO) << "[khadem] TypeAtCoordinates: Clicking at (" 
...
+  response.success = success;
+  
+  LOG(INFO) << "[khadem] TypeAtCoordinates: Result = " 
