...
+    LOG(INFO) << "khadem: ExtractPageContent - tree is empty";
+    return items;
+  }
+
+  LOG(INFO) << "khadem: ExtractPageContent - processing " << tree_update.nodes.size() << " nodes";
+
...
+
+  LOG(INFO) << "khadem: ExtractPageContent - extracted " << items.size() << " items";
+
+  return items;
+}
+
