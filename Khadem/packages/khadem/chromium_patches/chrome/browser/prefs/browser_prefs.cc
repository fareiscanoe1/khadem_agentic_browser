...
+#include "components/metrics/khadem_metrics/khadem_metrics_prefs.h"
...
+  khadem_server::RegisterLocalStatePrefs(registry);
+  khadem_metrics::RegisterLocalStatePrefs(registry);
...
+  khadem_metrics::RegisterProfilePrefs(registry);
...
+  registry->RegisterStringPref(prefs::kKhademProviders, "");
+  
+  // Legacy preferences (kept for backward compatibility)
+  registry->RegisterStringPref("nxtscape.default_provider", "khadem");
+  
...
+  
+  // Khadem toolbar settings
+  registry->RegisterBooleanPref(prefs::kKhademShowToolbarLabels, true);
+  
+  // Custom providers list - stored as a JSON string
+  registry->RegisterStringPref(prefs::kKhademCustomProviders, "[]");
+}
+
 #if BUILDFLAG(IS_CHROMEOS)
