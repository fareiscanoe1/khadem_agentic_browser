...
+// 9. Khadem extensions (AI Side Panel and Bug Reporter)
...
+         extension->id() == "egfdjlfmgnehecnclamagfafdccgfndp" ||
+         // Khadem extensions
+         extensions::khadem::IsKhademExtension(extension->id());
 }
...
+  // For Khadem extensions in sidepanel, allow audio for teach mode
+  if (extensions::khadem::IsKhademExtension(extension->id())) {
