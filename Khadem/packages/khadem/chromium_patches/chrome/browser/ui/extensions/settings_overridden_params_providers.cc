...
+  
+  // Don't show the dialog for Khadem extensions
+  for (const char* allowed_id : extensions::khadem::kAllowedExtensions) {
+    if (extension->id() == allowed_id) {
+      LOG(INFO) << "khadem: Skipping settings override dialog for Khadem extension " 
+                << extension->id();
+      return std::nullopt;
+    }
+  }
