...
+
+// Preference names
+const char kClashOfGptsPaneProvidersPref[] = "khadem.clash_of_gpts.pane_providers";  // Per-pane selections
+const char kClashOfGptsLastUrlsPref[] = "khadem.clash_of_gpts.last_urls";
+const char kClashOfGptsPaneCountPref[] = "khadem.clash_of_gpts.pane_count";
+
...
+    widget_->Show();
+    widget_->Activate();
+    khadem_metrics::KhademMetrics::Log("llmhub.shown");
+  }
+}
+
...
+    LOG(ERROR) << "[khadem] Invalid provider index: " << provider_index;
+    return;
+  }
+
...
+  SaveState();
+  
+  khadem_metrics::KhademMetrics::Log("llmhub.panecount.changed", 
