...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "khadem.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "khadem.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[khadem] Provider list size changed from " << previous_size
...
+  
+  khadem_metrics::KhademMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[khadem] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[khadem] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[khadem] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[khadem] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[khadem] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[khadem] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[khadem] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetBrowser().profile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[khadem] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[khadem] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  khadem_metrics::KhademMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  khadem_metrics::KhademMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    khadem_metrics::KhademMetrics::Log("llmchat.content.copied");
+    
...
+  LOG(INFO) << "[khadem] Media access requested for origin: "
...
+
+    LOG(INFO) << "[khadem] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[khadem] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[khadem] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
