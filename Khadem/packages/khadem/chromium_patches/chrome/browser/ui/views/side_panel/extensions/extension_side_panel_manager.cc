...
@@ -120,6 +122,14 @@ void ExtensionSidePanelManager::MaybeCreateActionItemForExtension(
           ...
           .Build());
+
+  // Auto-pin Khadem extensions to the toolbar.
+  if (khadem::IsKhademExtension(extension->id())) {
+    LOG(INFO) << "khadem: Auto-pinning Khadem extension: " << extension->id();
...
+  
+  // Unpin Khadem extensions before removing the action item
+  if (khadem::IsKhademExtension(extension->id())) {
+    LOG(INFO) << "khadem: Unpinning Khadem extension: " << extension->id() 
