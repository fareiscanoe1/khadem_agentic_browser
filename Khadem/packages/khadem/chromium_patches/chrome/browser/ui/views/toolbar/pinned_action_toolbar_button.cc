...
+#include "chrome/browser/ui/actions/khadem_actions_config.h"
...
+  // Update the text from the action item for Khadem actions
+  if (khadem::IsKhademAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
