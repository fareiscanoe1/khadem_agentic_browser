diff --git a/chrome/browser/khadem_server/validate_resources.py b/chrome/browser/khadem_server/validate_resources.py
new file mode 100644
index 0000000000000..d7dc82b132dad
--- /dev/null
+++ b/chrome/browser/khadem_server/validate_resources.py
@@ -0,0 +1,43 @@
+#!/usr/bin/env python3
...
+
+"""Validates that required Khadem resources exist.
+
...
+    print(f"ERROR: Required Khadem resource not found: {resource_path}")
+    all_valid = False
+    continue
+
...
+  sys.exit(1)
+
+print(f"✓ Khadem resources validated ({len(REQUIRED_RESOURCES)} resources)")
+sys.exit(0)
