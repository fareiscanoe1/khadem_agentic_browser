diff --git a/chrome/browser/khadem_server/khadem_server_manager.h b/chrome/browser/khadem_server/khadem_server_manager.h
new file mode 100644
index 0000000000000..d991d965c4913
--- /dev/null
+++ b/chrome/browser/khadem_server/khadem_server_manager.h
@@ -0,0 +1,138 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_KHADEM_SERVER_KHADEM_SERVER_MANAGER_H_
+#define CHROME_BROWSER_KHADEM_SERVER_KHADEM_SERVER_MANAGER_H_
+
+#include <memory>
+
...
+namespace khadem {
+
+// Khadem: Manages the lifecycle of the Khadem server process (singleton)
+// This manager:
+// 1. Starts Chromium's CDP WebSocket server (port 9222+, auto-discovered)
+// 2. Launches the bundled Khadem server binary with CDP and MCP ports
+// 3. Monitors MCP server health via HTTP /health endpoint and auto-restarts
+class KhademServerManager {
+ public:
+  static KhademServerManager* GetInstance();
+
+  KhademServerManager(const KhademServerManager&) = delete;
+  KhademServerManager& operator=(const KhademServerManager&) = delete;
+
+  // Starts the Khadem server if not already running
+  // This will:
...
+  // 4. Launch khadem_server binary with discovered ports
+  void Start();
+
+  // Stops the Khadem server
+  void Stop();
+
...
+
+ private:
+  friend base::NoDestructor<KhademServerManager>;
+
+  KhademServerManager();
+  ~KhademServerManager();
+
...
+  void StartCDPServer();
+  void StopCDPServer();
+  void LaunchKhademProcess();
+  void OnProcessLaunched(base::Process process);
+  void TerminateKhademProcess();
+  void RestartKhademProcess();
...
+
+  base::FilePath GetKhademServerResourcesPath() const;
+  base::FilePath GetKhademExecutionDir() const;
+  base::FilePath GetKhademServerExecutablePath() const;
...
+
+  base::WeakPtrFactory<KhademServerManager> weak_factory_{this};
+};
+
+}  // namespace khadem
+
+#endif  // CHROME_BROWSER_KHADEM_SERVER_KHADEM_SERVER_MANAGER_H_
