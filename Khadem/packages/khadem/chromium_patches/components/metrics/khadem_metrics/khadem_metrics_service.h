diff --git a/components/metrics/khadem_metrics/khadem_metrics_service.h b/components/metrics/khadem_metrics/khadem_metrics_service.h
new file mode 100644
index 0000000000000..cb7519c887826
--- /dev/null
+++ b/components/metrics/khadem_metrics/khadem_metrics_service.h
@@ -0,0 +1,95 @@
+// Copyright 2025 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef COMPONENTS_METRICS_KHADEM_METRICS_KHADEM_METRICS_SERVICE_H_
+#define COMPONENTS_METRICS_KHADEM_METRICS_KHADEM_METRICS_SERVICE_H_
+
+#include <memory>
+#include <string>
+
...
+class KhademMetricsService : public KeyedService {
+ public:
+  explicit KhademMetricsService(
+      PrefService* pref_service,
+      PrefService* local_state_prefs,
+      scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
+
+  KhademMetricsService(const KhademMetricsService&) = delete;
+  KhademMetricsService& operator=(const KhademMetricsService&) = delete;
+
+  ~KhademMetricsService() override;
+
...
+  base::WeakPtrFactory<KhademMetricsService> weak_factory_{this};
+};
+
+}  // namespace khadem_metrics
+
+#endif  // COMPONENTS_METRICS_KHADEM_METRICS_KHADEM_METRICS_SERVICE_H_
