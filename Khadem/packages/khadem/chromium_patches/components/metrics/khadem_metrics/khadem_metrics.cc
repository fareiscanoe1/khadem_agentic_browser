diff --git a/components/metrics/khadem_metrics/khadem_metrics.cc b/components/metrics/khadem_metrics/khadem_metrics.cc
new file mode 100644
index 0000000000000..02a1dc121bb7a
--- /dev/null
+++ b/components/metrics/khadem_metrics/khadem_metrics.cc
@@ -0,0 +1,100 @@
...
+KhademMetricsService* GetMetricsService() {
...
+  return KhademMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "khadem: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void KhademMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::Value::Dict(), sample_rate);
+}
+
+// static
+void KhademMetrics::Log(const std::string& event_name,
...
+
+// static
+void KhademMetrics::Log(const std::string& event_name, base::Value::Dict properties,
+                           double sample_rate) {
